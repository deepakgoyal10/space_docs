...
import { MdDownloading, MdOutlineDelete, MdOutlineDeleteForever } from "react-icons/md";
...
function Card({ data, foregroundRef, setDocOpen, setDocData }) {
    ...
    return (
        <motion.div drag whileDrag={{ scale: 1.1 }} dragElastic={0.2} dragConstraints={foregroundRef} dragTransition={{ bounceStiffness: 50, bounceDamping: 10 }} className=' relative w-60 h-72 bg-zinc-900/90 rounded-[50px] text-white px-5 py-10 overflow-hidden flex-shrink-0'>
            ...
            <p className='text-xs mt-2 font-semibold leading-tight '>{data.description?.length > 180 ? data.description?.slice(0, 180) + "..." : data.description}</p>
            <div className='absolute bottom-0 left-0  w-full   '>
                ...
                <div className='flex justify-end mb-3 mr-3'>
                    ...
                    <MdOutlineDeleteForever className='cursor-pointer' size={20} color='white' onClick={async () => await docs.remove(data.$id)} />
                </div>
                ...
                {/* </div> */}
                {
                    <div className='w-full py-4 flex items-center  justify-center cursor-pointer' style={{ backgroundColor: data.color, }} onClick={e => { setDocOpen(true); setDocData(data) }}>
